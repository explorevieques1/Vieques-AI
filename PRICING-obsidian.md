---
title: Explore Vieques — Pricing Model
tags:
  - pricing
  - explore-vieques
  - business-model
...
> [!abstract] Status
> **Proposal / working document.** Last updated 2026-07-25. Owner: Giancarlo.
...
- [[#7. Seasonality and revenue model]]
...
| **Exploration** | **\$24.99** | 30 days | 150 (fair use) | Anchor + power users, long stays, repeat visitors |
...
| **Basic** | **\$19** | \$190 | Small operators, kiosks, food trucks |
| **Featured** | **\$59** | \$590 | Restaurants, dive shops, tour operators |
| **Island Partner** | **\$149** | \$1,490 | Hotels, multi-location, ferry-adjacent, big operators |
...
> [!important] Headline change from today
> The current `\$9 / 30 days / unlimited AI` plan is both underpriced **and** a COGS risk. Replacing it with a three-tier ladder raises blended ARPU while capping AI exposure.
...
> [!important] The price ceiling is perception-bound, not value-bound
> That's why the sweet spot is \$10–15 for the main tier, and why Exploration at \$24.99 exists mostly to make Vacation look reasonable. Don't try to price to value here — you'll price yourself out.
...
The chat endpoint in `backend/server.js` runs a **tool-use loop** — typically 2–3 model calls per user question (initial → tool call → final answer), with tool results (beach rows, restaurant rows) pushed back into context. Realistic shape: **~8–12k input tokens, ~700 output tokens** across the whole loop.
...
> [!tip] Action item — biggest single margin win in this document
> The system prompt + tool definitions are identical on every request. Adding `cache_control` to that prefix cuts AI COGS roughly **60%**. Do this before launch — it's a one-line change.
...
> [!warning] Nothing below ~\$5
> This is the argument against a \$2.99 or \$3.99 entry tier **and** against the current \$3 credit pack — you'd hand Stripe 11–13% of it.
>
> It's also a quiet argument for annual business billing: \$190 once beats \$19 × 12 (you save ~\$3 in fixed fees plus 11 chances to churn).
...
| Vacation | \$12.99 | \$0.68 | \$0.30 (~10 of 25 used) | \$0.75 | **90%** |
| Exploration | \$24.99 | \$1.02 | \$1.20 (~40 of 150) | \$4.50 | **91%** (worst 78%) |
| Credit Pack | \$4.99 | \$0.44 | \$0.45 | \$0.90 | **82%** |
| Business Basic | \$19.00 | \$0.85 | \$0.00 | \$0.00 | **96%** |
| Business Featured | \$59.00 | \$2.01 | \$0.00 | \$0.00 | **97%** |

> [!note] Metered AI is not really your cost problem
> Even the worst-case Exploration user leaves 78% margin. The cap exists to prevent a pathological outlier (someone scripting 2,000 queries), not to protect typical margin.
>
> **This means you can be generous with message counts.** Generosity is cheap and converts well.
>
...
Current content inventory: **18 beaches, 35 restaurants, 23 essentials, 12 transport options**, snorkel spots + zones, PostGIS/OSRM routing. Activity and service listings are not yet populated — see [[#9. Open questions and risks]].
...
| Full beach profiles (facilities, 4×4, shade, surf) | ❌ | ✅ | ✅ | ✅ |
...
> [!danger] Free / Preview is non-negotiable
> Right now the map app is behind a hard paywall (`frontend/src/components/AccessGate.tsx`). **This is the single biggest conversion risk in the product.**
>
> Nobody pays \$13 for an app they've never seen, from a brand they've never heard of, for an island they're visiting once. The free tier isn't charity — it's the top of the funnel. Show the map, show pins with names, give 3 AI messages, then gate the detail.
...
**Exploration — \$24.99 / 30 days / 150 AI.** Two jobs:

1. Real value for long stays, repeat visitors, snowbirds, and people planning a group trip.
2. Mostly, it's a **decoy anchor** — with \$24.99 on the right, \$12.99 reads as the sensible middle rather than the expensive option. Classic center-stage effect. The itinerary builder and offline pack are what justify it, rather than "more AI."

### 4.2 Alternative: conservative launch ladder

If \$24.99 feels aggressive against a \$9 incumbent price:
...
> [!tip] I'd still start at the recommended numbers
> Raising prices later on a live product is harder than discounting. You can always run a launch promo (`LAUNCH30`) off the higher list price, which also gives you a real reference price for anchoring. Discounting down is easy; ratcheting up annoys early adopters.
...
### 5.1 The AI-recommendation integrity problem

> [!danger] Do not ship "Highlighted in AI recommendations" as currently written
> The current Featured tier promises it. The entire value of this product is that a traveler trusts what it tells them. The moment a paid tier can buy its way into an AI answer, you have two failure modes:
>
...
>
> This is not hypothetical — it's the exact mechanism that hollowed out TripAdvisor's and Yelp's credibility.
...
> Paid status may only break ties among results the ranker already considers equally relevant, and any paid surface must carry a visible label.
>
> Encode this in the **ranking layer**, not just in policy — and write it into the business ToS, so you're not tempted later when a hotel offers you \$500 to be "the top recommendation."
...
> [!tip] But hold the line at \$59 for Featured
> The pitch writes itself for anyone selling a ticketed experience:
>
...
**The free Claimed tier is the strategic core of the business side.** The sequence:
...
> [!warning] Copy to avoid
> Never say "unlimited" for Exploration without a fair-use footnote. Say **"Unlimited Ask AI\*"** with `*Fair use: 150 messages/month` in small print. Legally safer, and it's what every competitor does.
...
> [!important] Travelers are the audience; businesses are the revenue
> Traveler income is seasonal, one-time, and never compounds — every December you start from zero. Business subscriptions are smooth, recurring, and grow. But businesses only pay because travelers are there.
...
Deliberately conservative. Treat as an order-of-magnitude sanity check, not a forecast.
...
> That's a solid side business, not a venture outcome. The path to \$100k+ is:
>
> 1. Business subscriptions growing to 60–80 accounts, and
> 2. Replicating the platform to Culebra, then other Caribbean islands — the codebase is island-agnostic; the **content** is the moat.

---

## 8. Implementation notes

Mapping to the existing plan config in `backend/payments.js:66-68`:
...
| 8 | Ranking layer: paid status as **disclosed tiebreak only** | AI tool layer | 🔴 Blocker (see [[#5.1 The AI-recommendation integrity problem]]) |
| 9 | Business self-serve dashboard (edit listing, view stats) | New | 🟡 Medium |

> [!tip] The credit ledger already handles #1 and #2
> The append-only ledger in the schema does this cleanly — grant `aiMessages` as a positive ledger row on webhook, write a negative row per chat call, balance = `SUM(amount)`.
>
> See [[SECURITY-obsidian#🔴 Finding 1 — /api/ai/chat is completely unauthenticated]] — the credit system is currently decorative, and the AI route is unauthenticated. Both must be fixed before any of this metering is meaningful.
...
| 3 | Refund policy for weather/hurricane cancellations? | A traveler who buys a pass then cancels their trip will ask. Suggest: full refund if no AI messages used and <24h since purchase. |
...
| 5 | Multi-device enforcement mechanism? | Listed as a tier differentiator but currently unenforced. Either enforce it or drop it — an unenforced limit is just a lie in the pricing table. |
...
- [ ] Pricing page rebuilt for 3 traveler tiers + 4 business tiers (`landing/src/pages/Pricing.jsx`)

---

## Related

- [[SECURITY-obsidian]] — the AI route must be authenticated and metered before any of this pricing is enforceable
- [[CLAUDE]] — architecture, deployment state, known gaps
