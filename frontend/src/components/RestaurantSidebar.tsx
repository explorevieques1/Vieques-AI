...
function RestaurantSidebar({ activeSlug, onSelect, onClose }: Props) {
  const [categories, setCategories] = useState<RestaurantCategory[] | null>(null)
  ...
  return (
    <ResponsivePanel side="left" title="Restaurants" desktopWidth="sm:w-64" onClose={onClose}>
      <div className="px-4 py-3 border-b border-border">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-foreground">Restaurants</h2>
          <button
            onClick={onClose}
        ...
        <p className="text-xs text-muted-foreground mb-2">Pick a category to see places</p>
        <input
          value={query}
