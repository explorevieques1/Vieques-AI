...
import { fetchTransportCategories, type TransportCategory } from '../lib/api'
import { ResponsivePanel } from './ui/ResponsivePanel'
import { Skeleton } from './ui/skeleton'

type Props = {
  activeSlug: string | null
  onSelect: (slug: string, isPhysical: boolean) => void
  onClose: () => void
}

function TransportationSidebar({ activeSlug, onSelect, onClose }: Props) {
  const [categories, setCategories] = useState<TransportCategory[] | null>(null)
  ...
  return (
    <ResponsivePanel side="left" title="Transportation" desktopWidth="sm:w-64" onClose={onClose}>
      <div className="px-4 py-3 border-b border-border">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-foreground">Transportation</h2>
          <button
            onClick={onClose}
        ...
        <p className="text-xs text-muted-foreground mb-2">Pick a way to get around</p>
        <input
          value={query}
      ...
      <nav className="flex-1 overflow-y-auto scroll-contain p-2">
        ...
        {shown.map((c) => (
          <button
            key={c.slug}
            onClick={() => onSelect(c.slug, c.is_physical)}
        ...
        {categories !== null && shown.length === 0 && !error && (
          <div className="px-3 py-2 text-xs text-muted-foreground">No transportation matches.</div>
        )}
      </nav>
    </ResponsivePanel>
  )
